...
:return: A numpy array representing the statevector at the end of circuit evaluation
:rtype: numpy.ndarray
    """
    data = client_args_to_wire('circuits.run_statevector', **locals())
    api_call = post_call('circuits/run_statevector', data, host=host )
    logger.info(f'API call to circuits.run_statevector successful. Your API token is {api_call["uid"]}')
    return handle_result(wait_for_call(api_key=api_key,
                                       host=host,
                                       call_token=api_call['uid']))


async def async_run_statevector(backend:str, circuit:Circuit, statevector:numpy.ndarray=None, min_qubit:int=None, nqubit:int=None, dtype:type=numpy.complex128, backend_args:object={}, api_key:str=None, host:str=None):
    r"""Async version of run_statevector
Exceutes the circuit, returning a full statevector.  Only supported on some backends (for which `has_run_statevector` is true).  This is best used via the QuasarBackend class, which then allows a number of extensions.
