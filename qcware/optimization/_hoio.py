...
Contains the HOIO class. See ``help(qcware.optimization.HOIO)``.
...
def spin_var(name):
    ...
    Create a HOIO (see ``qcware.optimization.HOIO``) from a single spin variable.
    ...
    Return
    ------
    hoio : qcware.optimization.HOIO object.
        The model representing the binary variable.

    Examples
    --------
    >>> from qcware.optimization import spin_var, HOIO
    >>>
    >>> z0 = spin_var("z0")
    >>> print(z0)
    {('z0',): 1}
...
class HOIO(HIsing):
    ...
    ``help(qcware.optimization.HIsing)``.
    ...
    - The ``self.solve_bruteforce`` method will solve the HOIO ensuring that
      ...
      ``qcware.optimization.utils.solve_hising_bruteforce(self)`` or
      ``qcware.optimization.utils.solve_hising_bruteforce(self.to_pubo())`` will solve the
    ...
    Examples
    --------
    See ``qcware.optimization.HIsing`` for more examples of using HOIO without
    constraints. See ``qcware.optimization.HOBO`` for many constraint examples in PUBO
    ...
    def constraints(self):
        ...
        res : dict.
            ...
            ``'ge'``. The values are lists of ``qcware.optimization.HIsing`` objects. For
    ...
    def _append_constraint(self, key, constraint):
        ...
        constraint : qcware.optimization.HIsing object.
    ...
    def add_constraint_eq_zero(self,
        ...
        H : dict representing a HIsing.
            ...
            converted to a ``qcware.optimization.HIsing`` object if it is not already,
            thus it must follow the conventions, see ``help(qcware.optimization.HIsing)``.
