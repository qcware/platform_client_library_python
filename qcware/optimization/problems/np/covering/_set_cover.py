...
Contains the SetCover class. See ``help(qcware.optimization.problems.SetCover)``.

"""

from numpy import log2, allclose
from qcware.optimization.utils import QUBOMatrix, solve_qubo_bruteforce
...
class SetCover(Problem):
    ...
    ``qcware.optimization.problems.Problem`` class.
    ...
    def solve_bruteforce(self, all_solutions=False):
        ...
        Examples
        --------
        >>> from qcware.optimization import SetCover
        >>> U = {"a", "b", "c", "d"}
        >>> V = [{"a", "b"}, {"a", "c"}, {"c", "d"}]
        >>> problem = SetCover(U, V)
