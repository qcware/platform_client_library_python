...
Contains the DictArithmetic class. See ``help(qcware.optimization.utils.DictArithmetic)``.
It is used to define operations for dictionaries, and is a parent class to some
other classes, such as ``qcware.optimization.utils.QUBOMatrix`` and ``qcware.optimization.utils.BO``.
...
class DictArithmetic(dict):
    ...
    def normalize(self, value=1):
        ...
        Examples
        --------
        >>> from qcware.optimization.utils import DictArithmetic
