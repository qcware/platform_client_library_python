...
def ONE(x):
    ...
    Return
    ------
    P : ``qcware.optimization.PUBO`` object.
        The binary expression for the logic operation.

    Example
    -------
    >>> from qcware.optimization.sat import ONE
    >>> P = ONE('x')
    >>> P
    {('x',): 1}
    >>> P.value({'x': 1})
...
def NOT(x):
    ...
    Example
    -------
    >>> from qcware.optimization.sat import NOT
    >>> P = NOT('x')
    >>> P
    {(): 1, ('x',): -1}
    >>> P.value({'x': 1})
...
def AND(*variables):
    ...
    *variables : arguments.
    ...
    Example
    -------
    >>> from qcware.optimization.sat import AND
    >>> P = AND(0, 1)
    >>> P
    {(0, 1): 1}
    >>> P.value({0: 0, 1: 0})
...
def NAND(*variables):
    ...
    *variables : arguments.
    ...
    Example
    -------
    >>> from qcware.optimization.sat import NAND
    >>> P = NAND(0, 1)
    >>> P
    {(): 1, (0, 1): -1}
    >>> P.value({0: 0, 1: 0})
...
def OR(*variables):
    ...
    *variables : arguments.
    ...
    Example
    -------
    >>> from qcware.optimization.sat import OR
    >>> P = OR(0, 1)
    >>> P
    {(0,): 1, (0, 1): -1, (1,): 1}
    >>> P.value({0: 0, 1: 0})
...
def NOR(*variables):
    ...
    *variables : arguments.
    ...
    Example
    -------
    >>> from qcware.optimization.sat import NOR
    >>> P = NOR(0, 1)
    >>> P
    {(0,): -1, (0, 1): 1, (1,): -1, (): 1}
    >>> P.value({0: 0, 1: 0})
...
def XOR(*variables):
    ...
    if ``a == b``, otherwise it is 0. ``qcware.optimization`` uses the convention that
    ...
    *variables : arguments.
    ...
    Example
    -------
    >>> from qcware.optimization.sat import XOR
    >>> P = XOR(0, 1)
    >>> P
    {(0,): 1, (0, 1): -2, (1,): 1}
    >>> P.value({0: 0, 1: 0})
...
def XNOR(*variables):
    ...
    XOR(a, b) is 1 if ``a == b``, otherwise it is 0. ``qcware.optimization`` uses the
    ...
    *variables : arguments.
    ...
    Example
    -------
    >>> from qcware.optimization.sat import XNOR
    >>> P = XNOR(0, 1)
    >>> P
    {(): 1, (0,): -1, (0, 1): 2, (1,): -1}
    >>> P.value({0: 0, 1: 0})
