...
Contains the HOBO class. See ``help(qcware.optimization.HOBO)``.

"""

from . import PUBO
from .utils import QCWareWarning
...
def binary_var(name):
    ...
    Create a HOBO (see ``qcware.optimization.HOBO``) from a single binary variable.
    ...
    Return
    ------
    hobo : qcware.optimization.HOBO object.
        The model representing the binary variable.

    Examples
    --------
    >>> from qcware.optimization import binary_var, HOBO
    >>>
    >>> x0 = binary_var("x0")
    >>> print(x0)
    {('x0',): 1}
...
class HOBO(PUBO):
    ...
    - The ``self.solve_bruteforce`` method will solve the HOBO ensuring that
      ...
      ``qcware.optimization.utils.solve_pubo_bruteforce(self)`` or
      ``qcware.optimization.utils.solve_pubo_bruteforce(self.to_pubo())`` will solve the
    ...
    Examples
    --------
    See ``qcware.optimization.PUBO`` for more examples of using HOBO without constraints.
    ...
    >>> # or from qcware.optimization.utils import solve_qubo_bruteforce as qubo_solver
    >>> H = HOBO()
    >>>
    ...
    def constraints(self):
        ...
        res : dict.
            ...
            ``'eq'``, ``'lt'``, ``'le'``, ``'gt'``, and ``'ge'``.
            The values are lists of ``qcware.optimization.PUBO`` objects. For a
    ...
    def _append_constraint(self, key, constraint):
        ...
        constraint : qcware.optimization.PUBO object.
    ...
    def add_constraint_eq_zero(self,
        ...
        P : dict representing a PUBO.
            ...
            converted to a ``qcware.optimization.PUBO`` object if it is not already, thus
            it must follow the conventions, see ``help(qcware.optimization.PUBO)``.
    ...
    def add_constraint_eq_ONE(self, a, b, lam=1):
        ...
        >>> from qcware.optimization import HOBO, binary_var
    ...
    def add_constraint_XOR(self, *variables, lam=1):
        ...
        ..., ``v_n = variables[-1]``. See ``qcware.optimization.sat.XOR`` for the
        XOR convention that qcware.optimization uses for more than two inputs.
    ...
    def add_constraint_XNOR(self, *variables, lam=1):
        ...
        ``qcware.optimization.sat.XNOR`` for the XNOR convention that qcware.optimization uses for
