...
Contains the PUBO class. See ``help(qcware.optimization.PUBO)``.
...
class PUBO(BO, PUBOMatrix):
    ...
    deals with unconstrained qcware.optimizationimization problems that have arbitrary degree.
    ...
    def _reduce_degree(self, D, deg, lam):
        ...
        This is a private used only internally and in ``qcware.optimization.HIsing``. See
        ``qcware.optimization.PUBO.to_pubo``, ``qcware.optimization.HIsing.to_hising``,
        ``qcware.optimization.PUBO.to_qubo``, and ``qcware.optimization.HIsing.to_ising`` to see
        ...
        Parameters
        ----------
        D : ``qcware.optimization.utils.QUBOMatrix`` or ``qcware.optimization.utils.IsingMatrix``.
            The dictionary to fill. For reducing PUBOs to QUBOs, ``D`` should
            be a ``qcware.optimization.utils.QUBOMatrix`` object. For reducing HIsings
            to Isings, ``D`` should be a ``qcware.optimization.utils.IsingMatrix`` object.
        ...
        for key, v in mapped_self.items():
            ...
            while len(key) > deg:
                ...
                # enforce that z == x y
                D += qcware.optimization.HOBO().add_constraint_eq_AND(
